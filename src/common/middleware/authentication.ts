...
const authMiddleware = async (req: Request, res: Response, next: NextFunction) => {
    ...
    const data =await checkUser(authorization!)

    req.user = data.user
    req.decoded = { ...data.decoded, id: data.decoded.id.toString() }

    next()
};


export const authMiddleware_graphql = async (authorization: string) => {
    return await checkUser(authorization!)
};
